...
void QMC5883L::resetCalibration()
{
   ...
   {
      calibration.axes[axis].min = -32768;
      calibration.axes[axis].max =  32767;
   }  
}

void QMC5883L::recalcCalibration()
{
   byte  axis;
   long  deltas[3];
   float delta;
   ...
   for (axis = 0; axis < 3; axis++)
   {
      biases[axis]  = ((long)calibration.axes[axis].max + (long)calibration.axes[axis].min)/2;
      deltas[axis]  =  (long)calibration.axes[axis].max - (long)calibration.axes[axis].min   ;
      delta        += deltas[axis];      
   }
...
{
   return scales[axis] * (rawDataAxis(axis) - biases[axis]);
}
